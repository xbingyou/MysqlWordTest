...
namespace WindowsTest
{
    class WordEx
    {
        public static void MainWord()
        {
            ...
            string headerfile = Environment.CurrentDirectory + @"\3.png";
            ...
            //绘制横线方法一
            MSWord.Shape hLine, vLine;
            hLine = wordDoc.Shapes.AddConnector(Microsoft.Office.Core.MsoConnectorType.msoConnectorStraight, 0, 0, 432.108f, 0);
            vLine = wordDoc.Shapes.AddConnector(Microsoft.Office.Core.MsoConnectorType.msoConnectorStraight, 100, 100, 0, 499.147f);
            hLine.Line.Weight = 1.5f;
            vLine.Line.Weight = 0.75f;
            hLine.Top = 195.07f;
            hLine.Left = 0;
            vLine.Left = 360.09f;
            vLine.Top = 195.07f;
            hLine.Line.ForeColor.RGB = (int)MSWord.WdColor.wdColorLightBlue;
            vLine.Line.ForeColor.RGB = (int)MSWord.WdColor.wdColorRed;

            //绘制横线方法二
            //MSWord.Shape hLine;
            //object anchor = wordApp.Selection.Range;
            //float lineLeft = wordApp.CentimetersToPoints(0);
            //float lineTop = wordApp.CentimetersToPoints(1f);
            //float lineWidth = wordApp.CentimetersToPoints(15.56f);
            //hLine = wordDoc.Shapes.AddLine(lineLeft, lineTop, lineWidth, lineTop, ref anchor);
            //hLine.Line.Weight = 2f;
            //hLine.Line.ForeColor.RGB = (int)MSWord.WdColor.wdColorLightBlue;
            //hLine.RelativeVerticalPosition = MSWord.WdRelativeVerticalPosition.wdRelativeVerticalPositionParagraph;//相对于下边距
            //hLine.RelativeHorizontalPosition = MSWord.WdRelativeHorizontalPosition.wdRelativeHorizontalPositionColumn;
            //hLine.RelativeVerticalPosition = MSWord.WdRelativeVerticalPosition.wdRelativeVerticalPositionPage;
            ...
            //图片文件的路径
            string filename = Environment.CurrentDirectory + "\\6.png";
            //要向Word文档中插入图片的位置
